...
namespace Malam.Mastpen.Core.BL.Services
{
    public class EmployeeService : Service, IEmployeeService
    {
        ...
        public async Task<IPagedResponse<Employee>> GetEmployeesAsync(int pageSize = 10, int pageNumber = 1, int? EmployeeId = null)//, int? SiteId = null, DateTime? DateFrom = null, DateTime? DateTo = null)
        {

            var response = new PagedResponse<Employee>();

            // Get the "proposed" query from repository
            var query =DbContext.Employee.Include(b => b.IdentificationType);
              //.GetEmployee(); אם רוצים לפי סינונים מסוימים אז יש להשתמש בפונקציה
